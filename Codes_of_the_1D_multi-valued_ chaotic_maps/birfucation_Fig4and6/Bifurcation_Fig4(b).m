...
% Set the number of iterations for each 'a_I' value
len = 75;

% Initialize constant parameter 'b'
b = -0.95; 

% Iterate over 'a_I' values from 0.1 to 0.6 with a step size of 0.0008
for a = 0.1:0.0008:0.6
    % Set initial complex number
    x = 0.1 + 0.2i;
    
    % Iterate 10,000 times to reach a stable state
    for m = 1:10000
        x = (a * 1i + b) * complex_sqrt((x(1))^2, 2) + 1 + 0.5i;   
    end
    ...
    for m = 1:len
        ...
        % Store real, imaginary, and 'a_I' values for plotting
        X(m) = real(x);
        Y(m) = imag(x);
        A(m) = a;
    end 
...
set(gca, 'position', [0.11 0.29 0.87 0.68], 'FontName', 'Times New Roman', 'FontSize', 22, 'XTick', 0.1:0.1:0.6, 'YTick', -1.2:0.7:1.6);
xlabel('\it a_I', 'FontSize', 24, 'FontName', 'Times New Roman');
ylabel('\it x,y', 'FontSize', 24, 'FontName', 'Times New Roman');
axis([0.1 0.6 -1.2 1.6]);
